    ...
    (function (graphics) {
        var Bird = (function () {
            function Bird(field, forewing, hindwing, flight, _id) {
                ...
                this._g.append('line').attr('class', 'trunk').attr('x1', 5).attr('x2', 95).attr('y1', 90).attr('y2', 90);
                this._g.append('line').attr('class', 'beak').attr('x1', 85).attr('x2', 95).attr('y1', 90).attr('y2', 90);
                this._g.append('ellipse').attr('class', 'wing').attr('rx', 10).attr('ry', forewing * 3).attr('cx', 60).attr('cy', 90 - forewing * 3);
                this._g.append('ellipse').attr('class', 'wing').attr('rx', 10).attr('ry', hindwing * 3).attr('cx', 25).attr('cy', 90 - hindwing * 3);
                this._g.append('rect').attr('width', 100).attr('height', 100);
                this._g.on('click', function () {
                    d3.selectAll('g.bird > rect').attr('class', null);
        ...
        graphics.Bird = Bird;
        function Oasis(field, x, y) {
            var oasis = field.append('g');
            oasis.append('ellipse').attr('class', 'oasis').attr('rx', 80).attr('ry', 30).attr('cx', 40).attr('cy', 15);
            oasis.attr('transform', 'translate(' + x + ',' + y + ')');
        }
        graphics.Oasis = Oasis;
        function Field(parent, oasis) {
            parent.select('svg').remove();
            var field = parent.append('svg');
            field.append('rect').attr('class', 'oasis-' + oasis).attr('width', '100%').attr('height', '100%');
